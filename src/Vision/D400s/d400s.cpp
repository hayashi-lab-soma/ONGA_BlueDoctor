...
//        rsCfg->enable_stream(RS2_STREAM_DEPTH, 848,480, rs2_format::RS2_FORMAT_Z16, 30);
        rsCfg->enable_stream(RS2_STREAM_COLOR, 640,360, rs2_format::RS2_FORMAT_RGB8, 30);
        rsCfg->enable_stream(RS2_STREAM_DEPTH, 640,360, rs2_format::RS2_FORMAT_Z16, 30);
...
//convert [pixel] to [m]
int D400s::deproject2(int u, int v, double depth, float point[])
{
    try{
        float pixel[2] = {(float)u, (float)v};
        float d = depth;
